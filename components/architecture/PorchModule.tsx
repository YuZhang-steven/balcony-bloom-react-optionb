...
import { cScroll, greekKey, acanthus } from '../ironwork/primitives';
...
export function PorchModule({ cx, topY, moduleW, triH, triReach, hasKey }: PorchModuleProps) {
  ...
  return (
    <g className="porch-module">
      <IronStroke stroke={line([cx - halfW, topY], [cx + halfW, topY], { w: 6, color: IRON.deep })} />
      <IronStroke stroke={line([cx - halfW, topY - friezeH], [cx + halfW, topY - friezeH], { w: 4.5, color: IRON.mid })} />
      {hasKey && (
        <IronStroke stroke={greekKey(cx - halfW + 8, topY - friezeH + 4, moduleW - 16, friezeH - 8, { w: 3, color: IRON.deep })} />
      )}
      {!hasKey && (
          ...
          <MirrorH>
            {Array.from({ length: Math.floor(halfW / 32) }).map((_, i) => (
              <g key={i} transform={`translate(${12 + i * 32} 0)`}>
                <IronStroke stroke={cScroll(0, 0, 8, 1, 1.1, { w: 2.2, color: IRON.bright })} />
              </g>
            ))}
          </MirrorH>
        </g>
...
function TriangleBracket({ pilasterX, reach, h }: TriangleBracketProps) {
  const sweep = spline(
    [pilasterX, h],
    [
      { to: [pilasterX - reach * 0.5, h * 0.35], q: [pilasterX - reach * 0.18, h * 0.6] },
      { to: [pilasterX - reach, 2],              q: [pilasterX - reach * 0.82, h * 0.12] },
    ],
    { w: 4.5, color: IRON.deep },
  );
  ...
  const tendril = spline(
    [pilasterX - 2, h * 0.88],
    [
      { to: [pilasterX - reach * 0.06, h * 0.7],  q: [pilasterX - reach * 0.08, h * 0.82] },
      { to: [pilasterX - reach * 0.1,  h * 0.62], q: [pilasterX - reach * 0.03, h * 0.6]  },
    ],
  ...
  return (
    <g>
      <IronStroke stroke={sweep} />
      <IronStroke stroke={cScroll(sx, sy, sr, -1, 1.5, { w: 3 })} />
      <circle cx={sx} cy={sy} r={sr * 0.18} fill={IRON.deep} />
      <IronStroke stroke={cScroll(ex, ey, er, 1, 1.2, { w: 2.4, color: IRON.bright })} />
      <g transform={`translate(${ax} ${ay}) rotate(${aAng})`}>
        <IronFill d={acanthus(aLen)} color={IRON.bright} />
      </g>
      <IronStroke stroke={tendril} />
    </g>
  );
}
